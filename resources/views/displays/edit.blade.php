...
@section('content')
    <div class="block block-rounded">
        <div class="block-header block-header-default ">
            <h3 class="block-title">Editar Pack</h3>
        </div>
        <div class="block-content block-content-full d-flex justify-content-center">
            <div class="col-lg-8">
                ...
                <form action="{{ route('displays.update', $display->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-2 form-group">
                        ...
                        <select class="form-control" id="display_type" name="display_type" required>
                            <option value="six_beers" {{ $display->display_type == 'six_beers' ? 'selected' : '' }}>6 Cervezas</option>
                            <option value="twelve_beers" {{ $display->display_type == 'twelve_beers' ? 'selected' : '' }}>12 Cervezas</option>
                            <option value="twentyfour_beers" {{ $display->display_type == 'twentyfour_beers' ? 'selected' : '' }}>24 Cervezas</option>
                        </select>
                    </div>

                    <div class="mb-2 form-group">
                        <label class="form-label" for="name">Nombre del Pack</label>
                        <input type="text" class="form-control" id="name" name="name" placeholder="Nombre del pack" value="{{ $display->product->name }}" required>
                    </div>

                    <div class="mb-2 form-group">
                        <label class="form-label" for="description">Descripción</label>
                        <textarea class="form-control" id="description" name="description" placeholder="Descripción del producto" required>{{ $display->product->description }}</textarea>
                    </div>

                    <div class="mb-2 form-group">
                        <label class="form-label" for="value">Valor</label>
                        <input type="number" class="form-control" id="value" name="value" placeholder="Valor del pack" value="{{ $display->product->value }}" required>
                    </div>
                    ...
                    <div class="mb-2 form-group">
                        ...
                        <ul id="selected-beers" class="list-group">
                            @foreach($display->beers as $beer)
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    {{ $beer->product->name }}
                                    <button type="button" class="btn btn-danger btn-sm remove-beer" data-id="{{ $beer->id }}">Quitar</button>
                                    <input type="hidden" name="beers[]" value="{{ $beer->id }}">
                                </li>
                            @endforeach
                        </ul>
                    </div>
                    ...
                    <div class="mb-3 form-group">
                        ...
                        @if ($display->product->image)
                            <img src="{{ $display->product->image }}" alt="{{ $display->product->name }}" style="max-width: 100px; margin-top: 10px;">
                        @endif
                    </div>
