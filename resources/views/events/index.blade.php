...
<div class="content">
    ...
    <div class="block block-rounded">
        <div class="block-header block-header-default">
            <h3 class="block-title">Eventos<small></small></h3>
            <a href="{{ route('events.create') }}" class="btn btn-sm btn-outline-primary"> Crear </a>
            ...
            <table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">{{$event -> start_date_time}}</td>
                        <td class="text-center">{{$event -> end_date_time}}</td>
                        <td class="text-center">{{$event -> location_latitude}}</td>
                        <td class="text-center">{{$event -> location_longitude}}</td>
                        <td>
                            <!-- Nueva columna para botones de editar y eliminar -->
                            <div class="d-inline-flex">
                                <a href="{{ route('events.edit', $event->id) }}"
                                    class="btn btn-sm btn-outline-warning me-2">Editar</a>
                                <form action="{{ route('events.destroy', $event->id) }}" method="POST" class="formEliminar">
                                    @csrf
                                    @method('DELETE')
